...
function getInitialData() {
  const data = localStorage.getItem("dompetku-transaksi")
...
function App() {
  const [transaksi, setTransaksi] = useState(getInitialData)
  ...
  function handleSubmit(e) {
    e.preventDefault()
    const baru = { id: Date.now(), ...form, nominal: Number(form.nominal) }
    const updated = [baru, ...transaksi]
    setTransaksi(updated)
    localStorage.setItem("dompetku-transaksi", JSON.stringify(updated))
    setForm({
      keterangan: "",
      nominal: "",
      tipe: "pengeluaran",
          ...
          {dataHarian.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={dataHarian} margin={{ top: 4, right: 8, left: 8, bottom: 4 }}>
                <XAxis dataKey="tgl" tick={{ fill: "#aaa", fontSize: 11 }}
